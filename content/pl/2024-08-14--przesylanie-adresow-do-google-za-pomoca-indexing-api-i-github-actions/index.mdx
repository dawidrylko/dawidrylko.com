...
description: Archiwalny przykład integracji Google Indexing API z GitHub Actions wraz z aktualnym wyjaśnieniem, że API obsługuje wyłącznie strony ofert pracy i transmisji na żywo spełniające wymagania Google.
...
> **Aktualizacja z 7 lipca 2026 r.:** Google udostępniło Indexing API [26 czerwca 2018 r.](https://developers.google.com/search/blog/2018/06/introducing-indexing-api-for-job?hl=pl) dla stron z ofertami pracy. Obecnie API służy wyłącznie do zgłaszania aktualizacji lub usunięcia stron zawierających `JobPosting` albo `BroadcastEvent` osadzony w `VideoObject`, czyli transmisję na żywo. [Oficjalna dokumentacja](https://developers.google.com/search/apis/indexing-api/v3/using-api?hl=pl) nie przewiduje jego używania dla zwykłych podstron i wpisów blogowych. Ten artykuł pozostaje archiwalnym opisem dawnej implementacji; integracja została usunięta z aktualnego workflow. Dla pozostałych stron należy korzystać z mapy witryny i Google Search Console.
...
Ostatnią wersję konfiguracji sprzed usunięcia integracji znajdziesz [tutaj](https://github.com/dawidrylko/dawidrylko.com/blob/fead8a7a0d403faebb75ecbb3fd34f0a1687a60d/scripts/notify/google.sh).
...
```bash
chmod +x scripts/notify/google.sh
```
...
Ostatnią wersję workflow sprzed usunięcia integracji znajdziesz [tutaj](https://github.com/dawidrylko/dawidrylko.com/blob/fead8a7a0d403faebb75ecbb3fd34f0a1687a60d/.github/workflows/cd.yml#L80-L102).
