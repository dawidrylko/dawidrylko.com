---
title: Od tablicy do mapy haszującej ⚡️ Harry Potter i Transmutacja
description: Świadoma transmutacja danych to nie magia, lecz inżynieria. Artykuł pokazuje, jak i kiedy przekształcać tablice w mapy haszujące, analizując złożoność obliczeniową, koszty transformacji, wpływ na kod oraz realne benchmarki wydajności.
...
## Tablica → Mapa haszująca (_Array → Hash map_)

Jedną z najczęstszych i najbardziej fundamentalnych transformacji struktur danych jest przejście z **tablicy** na **mapę haszującą**. Zamiana ta uczy świadomego kompromisu: struktura danych powinna odpowiadać **charakterowi operacji**, a nie przyzwyczajeniom programisty.
...
tablica staje się niewystarczająca. W takich przypadkach lepszym wyborem jest **mapa haszująca** (_hash map_). Jest to struktura danych oparta na mechanizmie haszowania, która umożliwia **bezpośredni dostęp do wartości na podstawie klucza**.
...
### Przykład: Transformacja tablicy na mapę haszującą
...
```typescript
/**
 * @description Transformacja tablicy studentów na mapę haszującą dla szybkiego dostępu.
 */
...
```typescript
/**
 * @description Funkcja wyszukująca studenta w mapie haszującej na podstawie identyfikatora.
 * @param id - Identyfikator studenta do wyszukania.
 * @returns Znaleziony student lub undefined, jeśli nie znaleziono.
 */
const findStudentById = (id: string): Student | undefined => {
...
### Porównanie tablicy i mapy haszującej
...
Mapa haszująca nie jest rozwiązaniem uniwersalnym. Nie warto jej stosować, gdy:
...
Mapy haszujące sprzyjają więc nie tylko wydajności, ale również **prostocie logicznej kodu**.
...
- wyszukiwanie w mapie haszującej pozostaje praktycznie stałe czasowo.
...
Dla **100 studentów i 1 000 wyszukiwań** wszystkie podejścia są zbliżone czasowo. Różnice mieszczą się w ułamkach milisekund, a koszt transformacji do mapy haszującej sprawia, że przewaga `Map` jest jeszcze niewielka. W tym zakresie skali wybór struktury danych ma głównie znaczenie semantyczne, a nie wydajnościowe.
...
Transformacja **Tablica → Mapa haszująca** jest jedną z najważniejszych i najbardziej praktycznych przemian struktur danych. Uczy myślenia w kategoriach **intencji operacji**, a nie przywiązania do pierwotnej formy danych. To świadoma decyzja architektoniczna, która powinna być podejmowana zawsze wtedy, gdy:
...
- **Dostęp do danych**: Mapa haszująca umożliwia błyskawiczne wyszukiwanie elementów na podstawie klucza.
- **Deklaratywność**: Kod staje się bardziej czytelny i zrozumiały, gdy jasno określamy zamiar szybkiego dostępu do danych.
- **Struktura danych**: Mapa haszująca lepiej odzwierciedla charakter operacji wykonywanych na danych.
- **Złożoność cyklomatyczna**: Utrzymanie niskiej złożoności cyklomatycznej ułatwia zrozumienie i utrzymanie kodu.

Wady:

- **Zużycie pamięci**: Mapa haszująca wymaga więcej pamięci niż tablica.
- **Koszt transformacji**: Przekształcenie tablicy w mapę haszującą wiąże się z jednorazowym kosztem czasowym $O(n)$.
