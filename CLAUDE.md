...
Post metadata and images are the one exception, and only when the author asks for that work. Picking a featured image, swapping one, setting `featuredImg` and `featuredImgAlt`, adding or removing the image file next to `index.md(x)`: all of that is allowed on request. The canon for choosing the photo is the `/d-stock-images` skill, and it applies here in full. The agent downloads the file itself, from the address on the photo page, and never through the official Unsplash API: the API terms force a backlink to the photographer profile, which is why that route is banned in every project. An earlier version of this paragraph blamed ADR 0074 in `cyberkatalog-docs` for a ban on the agent downloading; that ADR never said so, and the wording was corrected on 2026-09-20. Everything else in that skill applies: the four rejects, the Polish audience, judging a candidate on the listing crop rather than the full frame, and the `<photographer>-<unsplash-id>-unsplash.jpg` filename. Frontmatter fields other than the image pair, and every line of prose in the body, stay the author's.
...
- **Homepage title** is built from `SITE_METADATA.title` plus `titleTagline`, short enough to stay under 60 with the brand. `titleTagline` currently repeats `author.jobTitle`, the positioning tagline behind Bio and JSON-LD Person and the same string used as the LinkedIn headline. Keep them as two fields and never alias one to the other: only `titleTagline` is bound by the 60-character title budget, so a longer headline must not reach `<title>` by accident.
